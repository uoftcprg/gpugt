...
1. Fork the GPUGT repository on GitHub.
2. Clone your fork locally: ``git clone git@github.com:uoftcprg/gpugt.git``
...
1. Run style checking: ``flake8 gpugt``
2. Run static type checking with ``--strict`` flag: ``mypy --strict gpugt``
3. Run checks for missing docstrings: ``interrogate -f 100 -i -m -n -p -s -r '^\w+TestCase' gpugt``
4. Run unit tests: ``python -m unittest``
5. Run doctests: ``python -m doctest gpugt/*.py``
...
3. Submit a pull request to the ``main`` branch in the GPUGT repository.
